# Mohd Farid Mohd Yusof

<img src="profile-pic.png" alt="photo" width="100"/>

> I'm a 34 years old Senior Software Engineer based in Malaysia. I love everything that has to do
with technology and learning new development stacks. I work closely with other colleagues
involved in projects and products. I'm able to adapt to bringing forth expertise in the system
design, implementation, testing and maintenance of software systems. Proficient in various
platforms, and languages, such as Laravel, NodeJS, Golang, React and VueJS for almost
6 years. Experienced with the latest cutting-edge development tools and procedures by
using Git, CI/CD, Ansible and Terraform. Able to effectively self-manage during independent
projects, as well as collaborate as part of a productive team.

You can download my complete resume here (Last updated: 28 October 2022)

[Download Resume](resume.pdf)

## ☎️ Contact information

📧  faridyusof727@gmail.com

🔗  [https://www.linkedin.com/in/faridyusof727/](https://www.linkedin.com/in/faridyusof727/)

telegram: [@faridyusof727](https://t.me/faridyusof727)

---

## **👩🏻‍💻** Work experience

### Junior Software Developer

Infineon Technologies Sdn Bhd*,* Melaka *– 2014-2015*

*I am responsible to develop the Response Monitoring Dashboard for Camstar (Manufacturing
Execution System) by using PHP, MySQL and Automate 6 (<https://www.helpsystems.com/products/automate-ultimate>).*

### Intermediate & Senior Software Developer

Amtis Solution Sdn Bhd*,* Melaka *– 2015-2017*

*I am responsible for developing and leading a team for several web/mobile required by
Government Sectors. One of them is MySocialStats <https://mysocialstats.stats.gov.my/> developed for the Department Of
Statistics, Malaysia. This system uses PHP Yii2, ElasticSearch and MySQL.*

Other contributions:

- <https://akademia.unimel.edu.my/>

### Senior Software Developer

Odekan Technologies PLT*, Cyberjaya Senior Fullstack DeveloperSenior Fullstack Developer– 2017-2019*

*I am responsible for developing and leading a team for several web/mobile required by
Government Sectors. One of them is MyCapstock <http://mycapstock.stats.gov.my/> developed for the Department Of
Statistics, Malaysia. This system uses PHP Yii2, MongoDB and Redis. We deploy to the
server using Docker technology.*

Other contributions:

- <https://play.google.com/store/apps/details?id=com.lpsvn.general.lpsvn_general>
  - Mobile: Flutter
  - Frontend: Vue3
  - Backend: Go Echo
- <https://myspr.spr.gov.my/login>
  - Exchange Module
  - Training Module

### Solution Architect

Raudhah Pay Sdn Bhd*, Cyberjaya – 2019-2020*

*I am responsible as a team lead and solution architect for Raudhah Pay payment gateway.
Raudhah Pay now has more than 5000 merchants all around Malaysia. For your reference,
you can surf [https://www.raudhahpay.com](https://www.raudhahpay.com/)*

### Senior Software Engineer

Easy Crypto*, New Zealand – 2021-current*

*Implement, enhance and upgrade <https://easycrypto.com> based on stakeholder
requirements. Train and groom developers to be at their best standards.*

- *Taking part in OKR discussion and being able to achieve most of the KRs*
- *Reduce CPU and RAM usage by refactoring code into workers*
- *Add a new payment gateway integration*
- *Supports and facilitates working methods from time to time*
- *Reduce technical debt by refactoring legacy codes*
- *Collaborate with cross-department team to develop web beta version*
  - *Working with design team to finalize ways of working*
  - *Following Component-Driven Development (Storybook)*
  - *Scaffolding Initial Code Base Structure*

---

## 🛠 Skills

### 💻 Technology

#### Backend Development ⭐️⭐️⭐️⭐️

My "native" programming language is now PHP (with and without framework) and JavaScript (NodeJS), I've worked with it for over 6 years. I've used it in most of my projects. I would love to switch to Golang as my next “native”. Currently learning Golang with Go Gin and Go Echo. Started by Golang journey in January 2021.

#### Frontend Development ⭐️⭐️⭐️

Started my lessons on jQuery. After months, I followed the trends of front-end frameworks such as React, Vue, and Angular. I was done some projects for around 3 years by using VueJS using Nuxt as its framework. I also improve in this area by following the [Component Driven Development](https://itnext.io/a-guide-to-component-driven-development-cdd-1516f65d8b55) approach since working with Easy Crypto.

#### DevOps ⭐️⭐️⭐️

During my year at Raudhah Pay, I spend my time working on DevOps. I am the person who designs the architecture of Raudhah Pay, making sure that the application is automatically scaled. We manage to have Kubernetes in our ecosystem with automated CI/CD working behind the scene while other devs work on the code and making PRs.

---

## 🗣 Languages

### English & Malay

---

## 📚 Education

### **Bachelor's Degree in Computer Science**

Year: *2014*

Universiti Teknikal Malaysia, Melaka - *Malaysian Technical University, Malacca*
